...
import NavlistMis from '../components/NavlistMis'; // Import NavList component
  ...
  const fetchTicketsAndStaff = async () => {
    try {
      ...
      const [ticketsResponse, staffResponse] = await Promise.all([
        axios.get('http://localhost:8080/TicketService/tickets', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }),
        axios.get('http://localhost:8080/MisStaffService/staff', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
